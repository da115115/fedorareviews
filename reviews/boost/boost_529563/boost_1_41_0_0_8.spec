...
# Delivery into the Python site_packages directory (https://fedoraproject.org/wiki/Packaging:Python)
...
Requires: boost-accumulators = %{version}-%{release}
...
#
# From the version 13 of Fedora, the Boost libraries are delivered with
# sonames equal to the Boost version (e.g., 1.41.0). Whereas on older versions
# of Fedora (e.g., Fedora 12), the Boost libraries are delivered with
# another scheme for sonames (e.g., a soname of 5 for Fedora 12).
%if 0%{?fedora} >= 13
...
# The following fixes have normally been fixed on the 1.41.0 branch/release
Patch0: boost-graph-compile.patch
#Patch1: boost-cmake4-soname.patch
#Patch2: boost-cmake-donot-build-tools.patch
Patch3: boost-cmake-soname-5.patch
...
Summary: Runtime component of boost accumulators library
Group: System Environment/Libraries

%description accumulators

Runtime support for Boost Accumulators, set of libraries for
...
%{__mkdir_p} build
cd build
%cmake -DENABLE_SINGLE_THREADED=YES -DCMAKE_BUILD_TYPE=RelWithDebInfo \
	   -DCMAKE_INSTALL_PREFIX=%{_prefix} -DINSTALL_VERSIONED=OFF ..
...
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_date_time*.so.%{sonamever}

%files filesystem
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_filesystem*.so.%{sonamever}

%files graph
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_graph*.so.%{sonamever}

%files iostreams
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_iostreams*.so.%{sonamever}
...
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_prg_exec_monitor*.so.%{sonamever}
%{_libdir}/lib%{name}_unit_test_framework*.so.%{sonamever}

%files program-options
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_program_options*.so.%{sonamever}

%files python
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_python*.so.%{sonamever}
...
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_regex*.so.%{sonamever}

%files serialization
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_serialization*.so.%{sonamever}
%{_libdir}/lib%{name}_wserialization*.so.%{sonamever}

%files signals
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_signals*.so.%{sonamever}

%files system
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_system*.so.%{sonamever}

%files thread
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_thread*.so.%{sonamever}

%files wave
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_wave*.so.%{sonamever}

%files mpi
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/lib%{name}_mpi*.so.%{sonamever}
...
%changelog
* Mon Dec 21 2009 Denis Arnaud <denis.arnaud_fedora@m4x.org> - 1.41.0-0.8
- Added the delivery of the mpi.so Python extension library
- Added the soname compatibility work-around for Fedora versions l.t. 12
