...
# Ticket submitted to Boost, and fixed in their development branch:
# https://svn.boost.org/trac/boost/ticket/3543
Patch14: boost-accumulators-namespace-fix.patch
# Patch required by Boost-CMake for version 1.39 only:
# http://gitorious.org/boost/cmake/blobs/raw/1.40.0/tools/build/CMake/docs/build/html/notes_by_version.html
Patch15: boost-cmake-minor_version_39.patch
...
# As stated by the official Boost-CMake documentation
# (http://gitorious.org/boost/cmake/blobs/raw/1.40.0/tools/build/CMake/docs/build/html/notes_by_version.html):
# "In versions 1.38 and 1.39, if you want to make install, you must make
# modularize first"
make modularize

# On 32bit platforms, Boost-configured CMake installs everything in the
# right directories. On 64bit platforms, however, the libraries are installed
# within the $RPM_BUILD_ROOT/usr/lib directory, where as they should be
# installed in the _BUILD_ROOT/usr/lib64 directory.
# Therefore, the following line could be uncommented iff:
# 1. The Boost-configured CMake installation is fixed for 64bit platforms.
# 2. The following part ("install lib" block) is removed, at least for the
#    libraries.
...
# install lib
for i in `find lib -type f -name \*.a`; do
  NAME=`basename $i`;
  install -p -m 0644 $i $RPM_BUILD_ROOT%{_libdir}/$NAME;
done;
for i in `find lib -type f -name \*.so`; do
  NAME=$i;
  SONAME=$i.%{sonamever};
  VNAME=$i.%{version};
  base=`basename $i`;
...
%defattr(-, root, root, -)
%{_includedir}/boost
#%%{_bindir}/accumulators-accumulators_example
#%%{_bindir}/bcp
%{_libdir}/*.so
...
%changelog
* Sun Oct 24 2009 Denis Arnaud <denis.arnaud_fedora@m4x.org> - 1.39.0-10
