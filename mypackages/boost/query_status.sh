...
rm -f boost_deplist_pack_task_all_success.txt boost_deplist_pack_task_all_failure.txt boost_deplist_pack_task_all_unfinished.txt
...
do
	...
	then
		...
		unset KOJI_CLOSED
		echo "${KOJI_STATUS}" | grep "State: failed" && KOJI_CLOSED=${_task}
		if [ -n "${KOJI_CLOSED}" ]
		then
