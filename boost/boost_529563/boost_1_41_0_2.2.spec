...
# Note: when the patch1 is applied, %%{version} should be replaced by
#       %%{sonamever} in the %%files section
#
%if 0%{?fedora} >= 13
...
%{_libdir}/libboost_mpi-mt.so.%{version}

%files mpi-python
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/libboost_mpi_python*.so.%{version}
