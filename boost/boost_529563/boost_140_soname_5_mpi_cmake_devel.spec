#
# Documentation: as the %%doc macro erases the target directory, namely
# $RPM_BUILD_ROOT%{_docdir}/%{name}-%{version}, manully installed documentation
# must be saved into a temporary dedicated directory.
...
Release: 1%{?dist}
License: Boost
URL: http://sodium.resophonic.com/boost-cmake/%{version}.cmake4/
Group: System Environment/Libraries
%define full_version %{name}-%{version}.cmake5
Source: %{full_version}.tar.bz2

# boost is an "umbrella" package that pulls in all other boost components
# Hence, the boost package itself does contain only documentation files
# (e.g., licence), and no binary. It may therefore be noarch.
#%%if 0%{?fedora} >= 10
#BuildArch: noarch
#%%endif
...
Patch0: boost-mpi-graph-compilation-fix.patch
# Temporary work-around, to set the soname equal to 5, so as to be compatible
# with the Boost version delivered on Fedora 12 (Boost version 1.39.0, with
# a soname version of 5).
Patch1: boost-cmake-soname-5.patch
...
%{__mkdir_p} build
cd build
%cmake -DENABLE_SINGLE_THREADED=YES ..
...
DESTDIR=$RPM_BUILD_ROOT make install VERBOSE=1
...
%{__rm} -rf %{mydocs} && %{__mkdir_p} %{mydocs}/html
...
%changelog
* Mon Dec 02 2009 Denis Arnaud <denis.arnaud_fedora@m4x.org> - 1.40.0-1
- Upstream integration (version 1.40.0)
- Backward compatibility with Fedora versions lower than or equal to 12

* Mon Nov 02 2009 Denis Arnaud <denis.arnaud_fedora@m4x.org> - 1.39.0-10
